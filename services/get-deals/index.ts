...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const locale =
      localeParam && LOCALES[localeParam] ? localeParam : detectLocale(originalUrl);
    ...
    const pubId = pubIdParam ?? (env[localeConfig.pubIdEnvKey] as string | undefined);
    ...
    if (accessKey && domain) {
      ...
      if (merchantResult.status === "fulfilled") {
        ...
        console.error("[get-deals] fetchMerchantCoupons error:", merchantResult.reason);
      }
    }
