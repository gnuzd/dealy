...
<div class="toast toast-bottom toast-center sm:toast-top sm:toast-end z-50 p-4">
  {#each store.toasts as toast (toast.id)}
    <div
      ...
      role="alert"
    >
      <svelte:component this={icons[toast.type]} class="w-6 h-6 shrink-0 {typeStyles[toast.type]}" />
      <span class="flex-1 font-semibold text-sm text-slate-700">{toast.message}</span>
      <button class="btn btn-ghost btn-xs btn-circle" onclick={() => store.removeToast(toast.id)}>
        <X size={16} />
      </button>
    </div>
  {/each}
