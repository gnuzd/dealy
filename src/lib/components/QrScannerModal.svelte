<script lang="ts">
  ...
  let { open = $bindable(false), onscanned }: {
    open: boolean;
  ...
  async function startCamera(el: HTMLVideoElement) {
    ...
    try {
      ...
      cameraError = 'Không thể truy cập camera. Vui lòng cho phép quyền camera hoặc thử upload ảnh.';
    }
  }
...
{#if open}
  ...
  <div class="modal modal-open" onclick={(e) => e.target === e.currentTarget && close()}>
    <div class="modal-box rounded-box max-w-md w-full flex flex-col gap-4">
      ...
      <div role="tablist" class="tabs tabs-boxed bg-base-200">
        <button
          ...
          onclick={() => { activeTab = 'camera'; cameraError = ''; fileError = ''; }}
        >
        ...
        <button
          ...
          onclick={() => { activeTab = 'file'; cameraError = ''; fileError = ''; }}
        >
      ...
      {#if activeTab === 'camera'}
        <div class="flex flex-col gap-3">
          <div class="rounded-box overflow-hidden bg-base-300 aspect-square flex items-center justify-center">
          ...
          {:else}
            <p class="text-base-content/50 text-xs text-center">Hướng camera vào QR code để quét tự động</p>
          {/if}
        </div>
      {:else}
        <div class="flex flex-col gap-3">
          <label class="flex flex-col items-center justify-center gap-3 border-2 border-dashed border-base-content/20 rounded-box p-8 cursor-pointer hover:border-primary/40 transition-colors">
            <Upload size={32} class="text-base-content/40" />
            <span class="text-base-content/60 text-sm text-center">Chọn ảnh chứa QR code</span>
