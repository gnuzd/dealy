<script lang="ts">
  ...
  const tabs: { id: Tab; label: string; icon: typeof Home }[] = [
    ...
    { id: "account", label: "Tài khoản", icon: CircleUser },
  ];
</script>
...
{#if store.platform !== "desktop"}
  <nav
    class={clsx(
      "bg-base-100 border-t border-base-300 rounded-t-3xl flex items-stretch shrink-0",
    ...
    {#each tabs as tab}
      ...
      <button
        class={clsx(
          ...
          active ? "text-primary" : "text-base-content/35 hover:text-base-content/60",
        )}
        onclick={() => store.setTab(tab.id)}
      >
        <svelte:component this={tab.icon} size={20} strokeWidth={active ? 2.5 : 1.8} />
        <span
          class={clsx(
